...
def try_run_solver(h, t, Ψx, Cw):
    try:
        return spsolve(create_coeff_matrix(h, Cw), create_const_vector(h, t, Ψx, Cw))
...
if sol_do_calibration_workflow:
    ...
    while t <= τ:
        ...
        hi[0] = upper_boundary_condition()
        hi[-1] = lower_boundary_condition(hi)
        h = try_run_solver(hm, t, Ψx, Cw((hi + hm)/2))

        # |Ax - b| == residual
        if sol_compute_residuals:
            residual = np.abs(np.asarray(np.matmul(create_coeff_matrix(h, Cw(h)).todense(), h)).reshape(-1) - create_const_vector(h, t, Ψx, Cw(h)))
   ...
   # ax3.plot([θr, θr], [z[0], z[-1]], label='Residual', color='red', linestyle='--', linewidth=0.5)
    ...
    for i,(timestamp, rain, m_Ψx) in enumerate(zip(ds_domain, ds_rain, ds_Ψx)): #reads rain from data 
        ...
        while t <= τ:
            ...
            int_rwu += np.abs(np.sum(compute_RWU(h, m_Ψx)))
            t += dt
    ...
    # # Compute T (transpiration)
    # transpiration = np.array(np_RWU) -
